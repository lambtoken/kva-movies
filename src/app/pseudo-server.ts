...
export class PseudoServer {
  ...
  public async getAllMovies(): Promise<Movie[]> {
    ...
    if (existingMovies.length > 0) {
      console.log('Loaded', existingMovies.length, 'movies from localStorage');
    ...
    try {
      ...
      console.log('Saved', movies.length, 'movies from API to localStorage');
      return movies;
    } catch (error) {
      ...
      console.log('Saved', randomMovies.length, 'random movies to localStorage');
  ...
  public clearMoviesDatabase(): void {
    ...
    console.log('Movies database cleared from localStorage');
  }
